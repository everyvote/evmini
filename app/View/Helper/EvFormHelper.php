...
class EvFormHelper extends AppHelper {
	 ...
	 *      Default text to be displayed in the input box
	 *
	 */
	public function selector($model, $pfx, $class = "", $action ="", $placeholder = "") {

		?>

		<div id="<?=$model.$pfx?>" class="SelectorHelper">
			<input type="text" <?= ($class != "")? ('class="'.$class.'"') : ('') ?><?= ($placeholder ? ' placeholder="' . htmlentities($placeholder) . '"' : '') ?> />
